		...
		if(RedSeleccionada == "true"){
			if($("#taskDescription textarea").val() != "" && $("#taskAuthor textarea").val() != "" && $("#taskDescription textarea").val() != "add description" && $("#taskAuthor textarea").val() != "author name"){
				if(editando == true){		
					...
					eventRefence.constraint = escape(tareaText); // defined below
					...
					newEvent = {
						...
						constraint: escape(tareaText2), // defined below
						color: '#257e4a',
...
function editEvents(evento){
	...
	$("#taskDescription textarea").val(unescape(evento.constraint));
	color = evento.color,
